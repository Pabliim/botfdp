...
const config = {
    XBOT: 'BOT HYPE', 
    instagram: 'https://www.instagram.com/pabliim.ip?r=nametag', 
    nomer: 'wa.me/5563991084396',
    youtube: 'OFF', 
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["556391084396@s.whatsapp.net"] 
...
case 'lofi':
            ...
            case 'criador':
            case 'keke':
                  client.sendMessage(from, {displayname: "Pablo", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'Este é o número do meu criador ^-^ ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
			...
			case 'remover':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Quem você quer que eu executo chefe?😡😡😡')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
